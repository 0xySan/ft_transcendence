export {};

declare function addListener(target: EventTarget, event: string, handler: EventListenerOrEventListenerObject): void;
declare function translateElement(language: string, element: HTMLElement): void;
declare function getUserLang(): string;
declare function loadPage(url: string): void;
declare function updateNavBar(userData: any): void;

const select = document.getElementById('method-select') as HTMLSelectElement;
const resendBtn = document.getElementById('resend-otp-btn') as HTMLButtonElement;

const VERIFY_TOTP_ENDPOINT = '/api/users/twofa/totp/token';
const VERIFY_EMAIL_ENDPOINT = '/api/users/twofa/email';
const VERIFY_BACKUP_ENDPOINT = '/api/users/twofa/backup-codes';
const LOGIN_ENDPOINT = '/api/users/accounts/login';
const SEND_EMAIL_ENDPOINT = '/api/users/twofa/email/send';

let methods = null as any[] | null; // will hold user's 2FA methods
let primary = null as any | null; // will hold primary 2FA method details
let resendCooldownTimer: number | null = null;

function inputEventOnInput(input: HTMLInputElement, index: number, inputs: HTMLInputElement[]): void {
    addListener(input, 'input', (e) => {
        const target = e.target as HTMLInputElement;
        const value = target.value;

        const forbidRegex = (primary && primary.method_type === 1) ? /[^0-9]/g : /[^0-9A-Za-z]/g;
        target.value = value.replace(forbidRegex, '').slice(0, 1);

        if (target.value && index < inputs.length - 1) {
            inputs[index + 1].focus();
        }

        if (inputs.every((i) => i.value.length === 1)) {
            const code = inputs.map((i) => i.value).join('');
            console.log('2FA code:', code);
        }
    });
}

function backspaceEventOnInput(input: HTMLInputElement, index: number, inputs: HTMLInputElement[]): void {
    addListener(input, 'keydown', (e) => {
        const event = e as KeyboardEvent;
        if (event.key === 'Backspace' && !input.value && index > 0) {
            inputs[index - 1].focus();
        }
    });
}

function pasteEventOnInput(input: HTMLInputElement, index: number, inputs: HTMLInputElement[]): void {
    addListener(input, 'paste', (e) => {
        const event = e as ClipboardEvent;
        e.preventDefault();
        const paste = event.clipboardData?.getData('text') ?? '';
        const onlyNumbers = /^\d+$/.test(paste);
        if (!onlyNumbers && primary.method_type == 1) return;

        const digits = paste.slice(0, inputs.length);
        if (!digits.length) return;

        digits.split('').forEach((char, i) => {
            if (inputs[i]) inputs[i].value = char;
        });

        const nextIndex = Math.min(digits.length, inputs.length - 1);
        inputs[nextIndex].focus();

        if (digits.length === inputs.length) {
            console.log('2FA code:', digits);
        }
        tryVerifyIfFilled(inputs);
    });
}

function enterKeyEventOnInput(input: HTMLInputElement, index: number, inputs: HTMLInputElement[]): void {
    addListener(input, 'keydown', (e) => {
        const ev = e as KeyboardEvent;
        if (ev.key === 'Enter') {
            const code = inputs.map(i => i.value).join('');
            if (code.length === inputs.length) verifyTwoFa(code);
        }
    });
}

async function patchUserToken(totpToken: string): Promise<void> {
    const inputs = Array.from(document.querySelectorAll<HTMLInputElement>('.otp-input'));
    inputs.forEach(i => i.disabled = true);

    try {
        const res = await fetch(LOGIN_ENDPOINT, {
            method: 'PATCH',
            credentials: 'include',
            headers: {
                'Content-Type': 'application/json',
                'accept-language': getUserLang()
            },
            body: JSON.stringify({ token: totpToken })
        });

        if (res.ok) {
            console.log('TOTP token patched successfully');
            return;
        }

        if (res.status === 401 || res.status === 400) {
            const err = await res.json().catch(() => ({ message: 'Invalid code' }));
            console.error('Verification error:', err);
            alert(err.message || 'Invalid 2FA code');
            inputs.forEach(i => i.value = '');
            if (inputs[0]) inputs[0].focus();
            return;
        }

        console.error('Verification failed, status', res.status);
        alert('Verification failed. Try again later.');
    } catch (e) {
        console.error('Network error during verification', e);
        alert('Network error. Try again.');
    } finally {
        inputs.forEach(i => i.disabled = false);
    }
}

function refreshNavBarState() {
    fetch("/api/users/me")
        .then(res => res.ok ? res.json() : null)
        .then(data => { updateNavBar(data); })
        .catch(err => { console.error("Error fetching user data:", err); updateNavBar(null); });
}

async function postEmailCode(code: string): Promise<Response> {
    return await fetch(VERIFY_EMAIL_ENDPOINT, {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'accept-language': getUserLang() },
        body: JSON.stringify({ uuid: primary ? primary.id : null, code })
    });
}

async function postTotpCode(code: string): Promise<Response> {
    return await fetch(VERIFY_TOTP_ENDPOINT, {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'accept-language': getUserLang() },
        body: JSON.stringify({ twofa_uuid: primary ? primary.id : null, totp_code: code })
    });
}

async function postBackupCode(code: string): Promise<Response> {
    return await fetch(VERIFY_BACKUP_ENDPOINT, {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'accept-language': getUserLang() },
        body: JSON.stringify({ uuid: primary ? primary.id : null, code })
    });
}

async function verifyTwoFa(code: string): Promise<void> {
    const inputs = Array.from(document.querySelectorAll<HTMLInputElement>('.otp-input'));
    inputs.forEach(i => i.disabled = true);

    try {
        let res: Response;
        switch (primary.method_type) {
            case 0: res = await postEmailCode(code); break;
            case 1: res = await postTotpCode(code); break;
            case 2: res = await postBackupCode(code); break;
            default: console.error('Unsupported 2FA type'); return;
        }

        if (res.ok) {
            let data: any = {};
            try { data = await res.json(); } catch (_) { console.warn('No JSON body returned from 2FA verify'); }
            const token = data.token ?? null;
            patchUserToken(token);
            refreshNavBarState();
            console.log('2FA verified');
            loadPage('/home');
            return;
        }

        if (res.status === 401 || res.status === 400) {
            const err = await res.json().catch(() => ({ message: 'Invalid code' }));
            console.error('Verification error:', err);
            alert(err.message || 'Invalid 2FA code');
            inputs.forEach(i => i.value = '');
            if (inputs[0]) inputs[0].focus();
            return;
        }

        console.error('Verification failed, status', res.status);
        alert('Verification failed. Try again later.');
    } catch (e) {
        console.error('Network error during verification', e);
        alert('Network error. Try again.');
    } finally {
        inputs.forEach(i => i.disabled = false);
    }
}

function populateMethodSelect() {
    console.log('Populating method select with methods:', methods);
    if (!select || !methods) return;

    if (methods.length <= 1) {
        select.parentElement?.classList.add('hidden');
        return;
    }

    select.innerHTML = '';
    methods.forEach((method, index) => {
        const option = document.createElement('option');
        option.value = index.toString();
        option.text = method.label;
        select.appendChild(option);
    });

    const primaryIndex = methods.findIndex(m => m.is_primary);
    if (primaryIndex >= 0) select.value = primaryIndex.toString();

    updateDataTranslateKeys();
}

function updateDataTranslateKeys() {
    const userLang = getUserLang();
    const titleElement = document.getElementById('twoFA-title');
    const descriptionElement = document.getElementById('twoFA-instructions');
    [titleElement, descriptionElement].filter((el): el is HTMLElement => el !== null)
        .forEach(element => {
            const baseKey = element.getAttribute('data-translate-key');
            if (!baseKey) return;
            let newKey = baseKey;
            switch (primary?.method_type) {
                case 0: newKey = baseKey.replace(/\.([^.]+)$/, '.email'); break;
                case 1: newKey = baseKey.replace(/\.([^.]+)$/, '.totp'); break;
                case 2: newKey = baseKey.replace(/\.([^.]+)$/, '.backupCodes'); break;
                default: newKey = baseKey.replace(/\.([^.]+)$/, '.unknown'); break;
            }
            element.setAttribute('data-translate-key', newKey);
            translateElement(userLang, element);
        });
}

function tryVerifyIfFilled(inputs: HTMLInputElement[]) {
    if (inputs.length === 0) return;
    if (inputs.every(i => i.value.length === 1)) {
        const code = inputs.map(i => i.value).join('');
        verifyTwoFa(code);
    }
}

// ----- NEW RESEND BUTTON LOGIC -----
function updateResendButton() {
    if (!resendBtn) return;
    if (primary?.method_type === 0) { // email
        resendBtn.classList.remove('hidden');
        resendBtn.disabled = false;
        resendBtn.setAttribute('data-translate-key', 'twofa.sendEmail');
        translateElement(getUserLang(), resendBtn);
    } else {
        resendBtn.classList.add('hidden');
    }
}

function startResendCooldown() {
    if (!resendBtn) return;
    resendBtn.disabled = true;
    let remaining = 60;
    resendBtn.textContent = `(${remaining}s)`;
    resendCooldownTimer = window.setInterval(() => {
        remaining -= 1;
        resendBtn.textContent = `(${remaining}s)`;
        if (remaining <= 0) {
            clearInterval(resendCooldownTimer!);
            resendCooldownTimer = null;
            resendBtn.disabled = false;
            resendBtn.setAttribute('data-translate-key', 'twofa.resendButton');
            translateElement(getUserLang(), resendBtn);
        }
    }, 1000);
}

function setupResendButton() {
    if (!resendBtn) return;

    addListener(resendBtn, 'click', async () => {
        if (!primary) return;

        try {
            const res = await fetch(SEND_EMAIL_ENDPOINT, {
                method: 'POST',
                credentials: 'include',
                headers: { 
                    'Content-Type': 'application/json', 
                    'accept-language': getUserLang() 
                },
                body: JSON.stringify({ uuid: primary.id })
            });

            if (!res.ok) {
                alert('Failed to send email. Try again later.');
                return;
            }

            startResendCooldown();
        } catch (err) {
            console.error(err);
            alert('Network error. Try again later.');
        }
    });
}

// ----- MAIN PREPARE INPUTS -----
function prepareInputs() {
    const inputs = Array.from(document.querySelectorAll<HTMLInputElement>('.otp-input'));
    console.log(inputs);

    fetch('/api/users/twofa/', { method: 'GET', credentials: 'include' })
        .then(async response => {
            if (response.status === 404) { loadPage('/home'); return null; }
            if (!response.ok) throw new Error('Failed fetching 2FA methods: ' + response.status);
            return response.json();
        })
        .then(data => {
            if (!data) return;
            // Filter only verified methods
            methods = (data.twoFaMethods ?? []).filter((m: any) => m.is_verified);
            if (!methods || methods.length === 0) { loadPage('/home'); return; }

            primary = methods.find((m: any) => m.is_primary);
            if (!primary) { loadPage('/home'); return; }
            console.log('Primary 2FA method found:', primary);

            populateMethodSelect();
            updateResendButton(); // show/hide resend button initially
        })
        .catch(err => { console.error(err); loadPage('/home'); });

    inputs.forEach((input, index) => {
        inputEventOnInput(input, index, inputs);
        backspaceEventOnInput(input, index, inputs);
        pasteEventOnInput(input, index, inputs);
        enterKeyEventOnInput(input, index, inputs);
        addListener(input, 'input', () => tryVerifyIfFilled(inputs));
    });

    addListener(select, 'change', () => {
        const selectedIndex = parseInt(select.value, 10);
        const selectedMethod = methods ? methods[selectedIndex] : null;
        if (selectedMethod) {
            primary = selectedMethod;
            console.log('Selected 2FA method changed to:', primary);
            updateDataTranslateKeys();
            updateResendButton();
            inputs.forEach(i => i.value = '');
            if (inputs[0]) inputs[0].focus();
        }
    });

    // Setup the resend button listener
    setupResendButton();
}

// Call prepareInputs() as before
prepareInputs();
