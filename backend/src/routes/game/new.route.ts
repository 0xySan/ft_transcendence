...
export function createGame(userId: string, reply: any, request: any): gameInterface {
	...
	return ({ gameId: newGameId, code: config.game.code, authToken: authToken.token });
}
...
export function createNewGameRoute(fastify: FastifyInstance) {
	...
	async (request, reply) => {
		...
		const game: gameInterface = createGame(userId, reply, request);
