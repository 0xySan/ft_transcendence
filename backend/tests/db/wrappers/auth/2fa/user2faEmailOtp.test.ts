...
describe("user2faEmailOtp wrapper - tests", () => {
	...
	it("Should return a list of user2faEmailOtp for given method IDs", () => {
		const insertMethod = db.prepare(`
			INSERT INTO user_2fa_methods (method_id, user_id, method_type, label, is_primary, is_verified)
			VALUES (?, ?, ?, ?, ?, ?)
		`);
		insertMethod.run(methodId, userId, 1, "email_restored", 1, 1);

		const methodId2 = uuidv7();
		insertMethod.run(methodId2, userId, 1, "email2", 0, 1);
		...
		createUser2faEmailOtp({
			method_id: methodId2,
			email: "dasdada@sdfsf.f",
			last_sent_code_hash: "hash2",
			last_sent_at: now,
			expires_at: now + 60000,
		});

		const otps = getUser2faEmailOtpsByMethodIds([methodId, methodId2]);
