...
describe("GameParticipants wrapper (DB constraints + cascade)", () => {
	...
	it("should add a participant to a game", () => {
		const participant = addParticipant(gameId, userId1, 10, "win", 1);
	...
	it("should not allow duplicate participants (UNIQUE(game_id, user_id))", () => {
		const first = addParticipant(gameId, userId1, 10, "win", 1);
		expect(first).toBeDefined();

		// second insertion for same (gameId, userId1) must fail or return undefined
		let second: unknown;
		try {
			second = addParticipant(gameId, userId1, 20, "loss", 2);
		} catch (err) {
	...
	it("should reject insertion with non-existing game or user (FK constraint)", () => {
		...
		let gotErr = false;
		try {
			addParticipant("9999999", userId1, 0, "draw");
		} catch (err) {
			gotErr = true;
		...
		if (!gotErr) {
			const res = addParticipant("9999999", userId1, 0, "draw");
		...
		gotErr = false;
		try {
			addParticipant(gameId, "9999999", 0, "draw");
		} catch (err) {
			gotErr = true;
			expect(String((err as Error).message)).toMatch(/FOREIGN KEY|constraint/i);
		}
		if (!gotErr) {
			const res = addParticipant(gameId, "9999999", 0, "draw");
			expect(res).toBeUndefined();
		}
	});

	it("should retrieve a participant by id", () => {
		const created = addParticipant(gameId, userId1, 10, "win", 1);
	...
	it("should return participants by game id", () => {
		addParticipant(gameId, userId1, 10, "win", 1);
		addParticipant(gameId, userId2, 20, "loss", 2);
	...
	it("should update participant fields (score/team/result)", () => {
		const p = addParticipant(gameId, userId1, 5, "draw", 1);
